...
namespace Testity.EngineServices
{
	...
	public abstract class LifetimeManagedGameObjectFactoryAdapter<TActualGameObjectType> : IEngineGameObjectFactoryService<IEngineGameObject>
		where TActualGameObjectType : class
	{
		...
		public LifetimeManagedGameObjectFactoryAdapter(ILifetimeManagedEngineObjectRegister<IEngineGameObject, TActualGameObjectType> lifetimeRegister)
		{
		...
		public IEngineGameObject Create(Vector3<float> position, Quaternion<float> rotation)
		{
			...
			if (concreteEngineObjectInstance == null)
				throw new InvalidOperationException(nameof(LifetimeManagedGameObjectFactoryAdapter<TActualGameObjectType>) + " failed to produce a valid " + nameof(TActualGameObjectType) + " via method: " + nameof(CreateActualEngineObject));

			IEngineGameObject bridgedGameObject = CreateGameObjectAdapter(concreteEngineObjectInstance);

			if (bridgedGameObject == null)
				throw new InvalidOperationException(nameof(LifetimeManagedGameObjectFactoryAdapter<TActualGameObjectType>) + " failed to produce a valid " + nameof(IEngineGameObject) + " vai method: " + nameof(CreateGameObjectAdapter));
